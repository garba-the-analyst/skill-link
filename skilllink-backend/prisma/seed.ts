...
// printed to the console below, so don't rely on them past your first login.
...
async function main() {
  console.log('Clearing database contents...');
  ...
  console.log('Generating cryptographic secure password representations...');
  ...
  for (const log of pendingLogs) {
    await prisma.volunteerHourLog.create({ data: { ...log, isVerified: false } });
  }
  ...
  // post-increment totalLoggedHours (4) — not a stale in-memory value — so
  // this is a genuine, verifiable hash rather than placeholder text.
  ...
  console.log('\n================================================================');
  console.log('  SKILLLINK MARKETPLACE SEEDED SUCCESSFULLY');
  console.log('================================================================');
  console.log(`- Super Admin login        : ${superAdmin.email} / ${ADMIN_PASSWORD}`);
  if (!process.env.SEED_ADMIN_EMAIL || !process.env.SEED_ADMIN_PASSWORD) {
    console.log('  (demo credentials — set SEED_ADMIN_EMAIL / SEED_ADMIN_PASSWORD in .env to override)');
  }
  console.log(`- Everyone else logs in with the password: ${DEMO_PASSWORD}`);
  console.log(`- 8 professionals seeded, each with a live listing (e.g. ${pro('tutor').user.email})`);
  console.log(`- 12 everyday users seeded (2 organizations, volunteers, and plain members)`);
  console.log('- 4 volunteer opportunities posted; 2 hour logs verified, 3 still pending review');
  console.log('- 4 bookings seeded, one at each escrow stage:');
  console.log(`    RELEASE_READY booking OTP (login as ${normal('musa').email}): ${demoOtp}`);
  console.log('================================================================\n');
}
